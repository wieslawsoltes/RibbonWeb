...
const CONTROL_TYPES = Object.freeze({
  button: 'button', toggleButton: 'toggle', checkBox: 'checkbox',
  comboBox: 'combobox', dropDown: 'dropdown', editBox: 'textbox',
  menu: 'menu', splitButton: 'split', gallery: 'gallery', dynamicMenu: 'menu',
  separator: 'separator', labelControl: 'label', item: 'button'
});
...
const finiteInteger = value => Number.isSafeInteger(Number(value)) && Number(value) >= 0 ? Number(value) : 0;

function compileRibbonXml(xml, callbacks = {}, options = {}) {
  if (typeof xml !== 'string') throw new TypeError('RibbonX XML must be a string.');
  if (xml.length > (options.maxXmlLength ?? 1_000_000)) throw new RangeError('RibbonX XML exceeds the configured size limit.');
  if (/<!DOCTYPE|<!ENTITY/i.test(xml)) throw new Error('RibbonX imports do not support document types or entity declarations.');
  const Parser = options.DOMParser || globalThis.DOMParser;
  if (!Parser) throw new Error('RibbonX requires a DOMParser. Use it in a browser, or supply options.DOMParser.');
  const warnings = [];
  ...
  function parse(source) {
    if (source.length > (options.maxXmlLength ?? 1_000_000)) throw new RangeError('RibbonX XML exceeds the configured size limit.');
    if (/<!DOCTYPE|<!ENTITY/i.test(source)) throw new Error('RibbonX imports do not support document types or entity declarations.');
    const document = new Parser().parseFromString(source, 'application/xml');
    if (!document.documentElement || document.getElementsByTagName('parsererror').length) throw new SyntaxError('Invalid RibbonX XML.');
    return document.documentElement;
  }
  const root = parse(xml);
  if (nameOf(root) !== 'customUI') throw new SyntaxError('RibbonX XML must have a customUI root element.');
  if (!SUPPORTED_NAMESPACES.has(root.namespaceURI)) warn('namespace', `Unrecognized RibbonX namespace: ${root.namespaceURI || '(none)'}.`);
  const index = new Map();
  ...
  function callback(node, attribute, descriptor) {
    ...
    if (own(callbacks, callbackName) && typeof callbacks[callbackName] === 'function') return callbacks[callbackName];
    const key = `${attribute}:${callbackName}`;
    if (!missingCallbacks.has(key)) {
      missingCallbacks.add(key);
      warn('missing-callback', `${attribute} references an unregistered callback "${callbackName}".`, descriptor.id);
    }
    return null;
  }
  function invoke(fn, descriptor, ...args) {
    if (disposed || !fn) return undefined;
    try { return fn(descriptor, ...args); }
    catch (error) {
  ...
  function synchronous(value, descriptor) {
    if (value && typeof value.then === 'function') {
      ...
      warn('async-getter', 'RibbonX value getters must return synchronously. Dynamic getContent supports promises through getItems().', descriptor.id);
      return undefined;
    }
    return value;
  }
  function register(node, target, fallback) {
    const id = node.getAttribute('id') || node.getAttribute('idQ') || node.getAttribute('idMso') || `${fallback}-${++nextId}`;
    if (index.has(id)) throw new SyntaxError(`Duplicate RibbonX control id: ${id}.`);
    target.id = id;
    const descriptor = Object.freeze({
      id, Id: id, tag: node.getAttribute('tag') || '',
    ...
    if (descriptor.idMso) {
      ...
      warn('built-in-control', 'idMso identifies an Office built-in. Its implementation, label, icon, and command must be supplied by the host.', id);
    }
    return descriptor;
  }
  function bind(node, attribute, target, property, descriptor, convert = value => value) {
    ...
    const refresh = () => {
      ...
      if (value !== undefined) { target[property] = convert(value); changed(target, property); }
    };
  ...
  function common(node, target, descriptor) {
    ...
    if (node.hasAttribute('size')) target.size = node.getAttribute('size') === 'large' ? 'large' : 'small';
    ...
    if (node.hasAttribute('image')) {
      ...
      if (typeof options.resolveImage === 'function') target.icon = options.resolveImage(source, descriptor);
      else warn('image-resource', `Image resource "${source}" needs options.resolveImage to return a web icon.`, descriptor.id);
    }
    if (node.hasAttribute('imageMso')) warn('built-in-image', `Office image "${node.getAttribute('imageMso')}" is not bundled. Supply a web icon with options.resolveImage.`, descriptor.id);
    ...
    bind(node, 'getSize', target, 'size', descriptor, value => value === 'large' || value === 1 ? 'large' : 'small');
    bind(node, 'getImage', target, 'icon', descriptor, value => typeof options.resolveImage === 'function' ? options.resolveImage(value, descriptor) : String(value));
    for (const attribute of ['insertAfterMso', 'insertBeforeMso', 'insertAfterQ', 'insertBeforeQ']) {
      if (node.hasAttribute(attribute)) warn('office-placement', `${attribute} is not applied; imported controls retain XML order.`, descriptor.id);
    }
    if (node.hasAttribute('sizeString')) warn('size-string', 'sizeString is an Office text-measurement hint. Set a web control width explicitly if needed.', descriptor.id);
  }
  function replaceContent(target, newItems) {
    ...
    function forget(item) {
      ...
      if (index.get(item.id) === item) { index.delete(item.id); bindings.delete(item.id); }
    }
  ...
  function controls(nodes) {
    return nodes.flatMap(node => {
      ...
      if (kind === 'box' || kind === 'buttonGroup') {
        warn('flattened-layout', `${kind} is flattened into its parent. Office-specific nested layout is not reproduced.`, node.getAttribute('id') || '');
        return controls(children(node));
      }
      if (!own(CONTROL_TYPES, kind)) {
        warn('unsupported-element', `The RibbonX ${kind} element is not supported.`, node.getAttribute('id') || '');
        return [];
      }
      ...
      if (kind === 'editBox' || kind === 'comboBox') {
        target.value = '';
        if (node.hasAttribute('maxLength')) target.maxLength = finiteInteger(node.getAttribute('maxLength'));
      ...
      if (kind === 'dropDown' || kind === 'comboBox' || kind === 'gallery') {
        ...
        if (getItemCount) {
          const refreshItems = () => {
            ...
            if (count > maxItems) throw new RangeError(`RibbonX item count exceeds ${maxItems} for ${descriptor.id}.`);
            target.items = Array.from({ length: count }, (_, i) => {
              ...
              const item = { id: String(idValue ?? `${descriptor.id}-item-${i}`), label: String(labelValue ?? `Item ${i + 1}`) };
              if (getItemImage) {
                const value = synchronous(invoke(getItemImage, descriptor, i), descriptor);
                if (value != null) item.icon = typeof options.resolveImage === 'function' ? options.resolveImage(value, descriptor) : String(value);
              }
              return item;
            });
            changed(target, 'items');
        ...
        if (getSelectedItemID || getSelectedItemIndex) {
          const refreshSelection = () => {
            ...
            else if (selectedIndex !== undefined) target.value = target.items[finiteInteger(selectedIndex)]?.id ?? '';
            if (id !== undefined || selectedIndex !== undefined) changed(target, 'value');
          };
          bindings.get(descriptor.id).push(refreshSelection);
          refreshSelection();
        }
        if (node.hasAttribute('columns')) target.columns = Math.max(1, finiteInteger(node.getAttribute('columns')));
        for (const hint of ['itemWidth', 'itemHeight', 'rows']) {
          if (node.hasAttribute(hint)) warn('gallery-layout-hint', `${hint} is not mapped to an exact Office gallery metric.`, descriptor.id);
        }
      }
      if (kind === 'splitButton') {
        ...
        if (primary) {
          const inherited = ['label', 'icon', 'enabled', 'visible', 'tooltip', 'description', 'keyTip'].filter(key => target[key] === undefined);
          const refreshPrimary = () => {
            for (const key of inherited) if (primary[key] !== undefined) { target[key] = primary[key]; changed(target, key); }
      ...
      if (kind === 'dynamicMenu') {
        ...
        if (getContent) {
          const applyContent = content => {
            ...
            if (typeof content !== 'string') throw new TypeError(`getContent for ${descriptor.id} must return RibbonX menu XML.`);
            const menu = parse(content);
            if (nameOf(menu) !== 'menu') throw new SyntaxError('RibbonX getContent must return a menu root element.');
      ...
      if (onAction) {
        const action = (parameter, liveControl = target) => {
          ...
          if (kind === 'dropDown' || kind === 'gallery') {
            ...
            const selection = Array.from(items).findIndex(item => item.id === target.value || item.value === target.value);
            return invoke(onAction, descriptor, selection < 0 ? String(target.value ?? '') : items[selection].id, selection);
          }
  ...
  function tab(node, contextual = null) {
    ...
    if (node.hasAttribute('getLabel')) bindings.get(descriptor.id).push(() => { target.header = target.label || descriptor.id; changed(target, 'header'); });
    for (const groupNode of children(node)) {
      if (nameOf(groupNode) !== 'group') {
        warn('unsupported-element', `Only groups can be direct children of an imported tab; ${nameOf(groupNode)} was skipped.`, descriptor.id);
        continue;
      }
      const group = { items: [] };
      ...
      if (groupNode.hasAttribute('getLabel')) bindings.get(groupDescriptor.id).push(() => { group.header = group.label || groupDescriptor.id; changed(group, 'header'); });
      group.items = controls(children(groupNode).filter(child => nameOf(child) !== 'dialogBoxLauncher'));
      ...
      if (launcher) {
        ...
        if (launcherItems.length) {
          ...
          warn('dialog-launcher-layout', 'The dialog launcher is included as a regular group button.', group.id);
        }
      }
  ...
  function ribbon(node) {
    if (node.getAttribute('startFromScratch') === 'true') warn('host-chrome', 'startFromScratch concerns Office host chrome and has no additional effect in a standalone ribbon.');
    for (const child of children(node)) {
      switch (nameOf(child)) {
        case 'tabs': model.tabs.push(...children(child).filter(item => nameOf(item) === 'tab').map(item => tab(item))); break;
        case 'qat':
          for (const area of children(child)) {
            if (['sharedControls', 'documentControls'].includes(nameOf(area))) model.quickAccessToolbar.push(...controls(children(area)));
            else warn('unsupported-element', `Unsupported Quick Access Toolbar element: ${nameOf(area)}.`);
          }
          break;
        case 'contextualTabs':
          for (const set of children(child)) {
            const contextId = set.getAttribute('id') || set.getAttribute('idMso') || `context-${++nextId}`;
            if (set.hasAttribute('idMso')) warn('built-in-context', `Context ${contextId} must be activated by the web host using setContext().`, contextId);
            model.tabs.push(...children(set).filter(item => nameOf(item) === 'tab').map(item => tab(item, contextId)));
          }
          break;
        case 'officeMenu': model.backstage.push(...controls(children(child))); break;
        default: warn('unsupported-element', `Unsupported ribbon element: ${nameOf(child)}.`);
      }
    }
  }
  for (const child of children(root)) {
    switch (nameOf(child)) {
      case 'ribbon': ribbon(child); break;
      case 'backstage':
        model.backstage.push(...controls(children(child)));
        warn('backstage-layout', 'Simple Backstage commands are imported. Office Backstage form layouts and host document workflows are not reproduced.');
        break;
      case 'commands': warn('command-repurposing', 'Repurposing built-in Office commands is not supported. Bind a web command explicitly.'); break;
      case 'contextMenus': warn('context-menus', 'Office host context-menu extensions are not imported into the ribbon.'); break;
      default: warn('unsupported-element', `Unsupported customUI element: ${nameOf(child)}.`);
    }
  }
  if (root.hasAttribute('loadImage')) warn('load-image', 'loadImage is replaced by the explicit options.resolveImage web image resolver.');
  if (root.hasAttribute('onLoad') && options.ignoreOnLoad) warn('onload', 'onLoad is only invoked by createRibbonXAdapter(), where an invalidation API is available.');
  ...
  return {
    model, warnings, index, invalidate, updates,
    onLoad: root.getAttribute('onLoad') ? callback(root, 'onLoad', Object.freeze({ id: 'customUI', Id: 'customUI', tag: '' })) : null,
    dispose() { disposed = true; bindings.clear(); index.clear(); updates.clear(); }
  };
}
...
export function createRibbonXAdapter(xml, callbacks = {}, options = {}) {
  ...
  const api = {
    ...
    attach(element) {
      ...
      if (!element || typeof element !== 'object') throw new TypeError('attach() requires a ribbon element.');
      ribbon = element;
      ribbon.model = api.model;
      return api;
    },
    dispose() { disposed = true; ribbon = null; compiled.dispose(); }
  };
