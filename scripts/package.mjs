...
async function walk(path, base = root) {
  ...
  if (info.isFile())
    return [{ name: relative(base, path).replaceAll('\\', '/'), data: await readFile(path) }];
  const result = [];
  for (const entry of (await readdir(path, { withFileTypes: true })).sort((a, b) =>
    ...
    if (
      ...
      ['node_modules', '.git', 'artifacts', 'site', 'bin', 'obj'].includes(entry.name)
    )
      continue;
...
export async function packageRelease() {
  ...
  const browserFiles = (
    await Promise.all(
      ['dist', 'src', 'examples', 'docs', 'LICENSE', 'README.md'].map((name) =>
        walk(resolve(root, name)),
      ),
    )
  ).flat();
  browserFiles.push({
    name: 'index.html',
    data: Buffer.from(
      '<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width"><meta http-equiv="refresh" content="0;url=./examples/"><title>RibbonWeb</title><a href="./examples/">Open the RibbonWeb showcase</a></html>\n',
    ),
  });
  await writeFile(
    resolve(output, `ribbon-web-${pkg.version}-browser.zip`),
    createZip(browserFiles),
  );
  const sourceFiles = (
    await Promise.all(
      [
        ...
        '.gitignore',
        '.prettierrc.json',
      ].map((name) => walk(resolve(root, name))),
    )
  ).flat();
  ...
  console.log(
    `Release artifacts: ${manifest.files.map((file) => `${file.name} (${file.bytes} bytes)`).join(', ')}`,
  );
  return manifest;
}
