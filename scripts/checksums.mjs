...
await writeFile(resolve(root,'artifacts/SHA256SUMS.txt'),files.map(f=>`${f.sha256}  ${f.name}`).join('\n')+'\n');
await writeFile(resolve(root,'artifacts/manifest.json'),JSON.stringify({name:pkg.name,version:pkg.version,format:'esm',npm:files.find(f=>f.name.endsWith('.tgz'))?.name,files},null,2)+'\n');
