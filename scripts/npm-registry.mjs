...
export function assertRegistryArtifact(metadata, pkg, integrity) {
  ...
  assert.equal(
    ...
    integrity,
    'Published version has different bytes; immutable versions cannot be replaced',
  );
  ...
  assert.equal(
    tarball.username + tarball.password,
    '',
    'Registry tarball URL must not contain credentials',
  );
  return tarball;
}
...
export async function fetchRegistryJson(url, fetcher = fetch) {
  ...
  assert(
    ...
    'Registry returned malformed metadata',
  );
  return metadata;
}
...
async function prepare(directory) {
  ...
  assert.equal(
    ...
    entries[0].digest,
    'Release tarball checksum mismatch',
  );
...
async function verify(tarball, distTag) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      const tags = await fetchRegistryJson(
        `${registry}/-/package/${encodeURIComponent(pkg.name)}/dist-tags`,
      );
      if (tags?.[distTag] !== pkg.version)
        throw new Error(
          `npm ${distTag} currently points to ${tags?.[distTag] ?? 'no version'}; waiting for ${pkg.version}`,
        );
      const packument = await fetchRegistryJson(packageUrl);
      const indexedVersion = packument?.versions?.[pkg.version];
      if (!indexedVersion)
        throw new Error('Published version is not visible in the npm install package index yet');
      assertRegistryArtifact(indexedVersion, pkg, expectedIntegrity);
      if (packument?.['dist-tags']?.[distTag] !== pkg.version)
        throw new Error(`npm install package index ${distTag} is not updated yet`);
      if (!metadata.dist.attestations?.provenance)
        throw new Error('npm provenance metadata is not visible yet');
      lastError = undefined;
      break;
    } catch (error) {
  ...
  assert.equal(
    integrityOf(bytes),
    expectedIntegrity,
    'Downloaded npm tarball differs from the release',
  );
  ...
  try {
    ...
    const result = spawnSync(
      process.execPath,
      [join(project, 'scripts/package-test.mjs'), '--tarball', downloaded],
      {
        cwd: project,
        stdio: 'inherit',
        timeout: 180_000,
    ...
    assert.equal(
      result.status,
      0,
      `Published npm consumer verification failed: ${result.error?.message ?? result.signal ?? result.status}`,
    );
    ...
    await writeFile(
      ...
      JSON.stringify({
        name: 'ribbonweb-public-registry-consumer',
        version: '1.0.0',
        private: true,
        type: 'module',
      }),
    ...
    const installed = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...
          NODE_AUTH_TOKEN: '',
          NPM_TOKEN: '',
          NPM_CONFIG_USERCONFIG: join(fresh, '.npmrc'),
        },
      },
    );
    assert.equal(
      installed.status,
      0,
      `Fresh public npm install by package name failed: ${installed.error?.message ?? installed.status}`,
    );
    const smoke = spawnSync(
      ...
      [
        '--input-type=module',
        '-e',
        `import assert from 'node:assert/strict'; import { ObservableObject, RibbonModel } from ${JSON.stringify(pkg.name)}; import { ObservableObject as CoreObject } from ${JSON.stringify(pkg.name + '/core')}; assert.equal(ObservableObject, CoreObject); const model = new ObservableObject({ Title: 'Installed' }); model.Title = 'Verified'; assert.equal(model.Title, 'Verified'); assert(new RibbonModel() instanceof CoreObject);`,
      ],
      { cwd: fresh, stdio: 'inherit', timeout: 30_000 },
    );
    assert.equal(
      smoke.status,
      0,
      'Fresh npm installation must preserve main/core class identity and execute observable models',
    );
    console.log(
      `Verified public ${pkg.name}@${pkg.version}: integrity, ${distTag} tag, package index, provenance metadata, downloaded tarball, installed consumers, and fresh npm installation.`,
    );
    await summary(
      `Published and verified **${pkg.name}@${pkg.version}** on npm (${distTag}).\n\nRelease and registry tarballs have identical SHA512 integrity. Public download, provenance metadata, ESM, CommonJS dynamic imports, strict TypeScript consumers, main/core class identity, MVVM commands and bindings, packaged styles and declarations, and a fresh npm installation checks passed.\n\n[View npm package](https://www.npmjs.com/package/${pkg.name}/v/${pkg.version})`,
    );
  } finally {
...
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  ...
  assert(
    path,
    'Usage: node scripts/npm-registry.mjs prepare <release-directory> | check <tarball> | verify <tarball> [latest|next]',
  );
