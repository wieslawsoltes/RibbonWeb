...
export async function build() {
  ...
  for (const name of ['index.js', 'core.js', 'ribbon.js', 'ribbonx.js', 'dotnet.js']) {
    if (!(await exists(resolve(root, 'src', name))))
      throw new Error(`Missing source module: src/${name}`);
  }
  ...
  await writeFile(
    ...
    JSON.stringify({ name: pkg.name, version: pkg.version, format: 'esm' }, null, 2) + '\n',
  );
  console.log(
    `Built ${pkg.name}@${pkg.version}: native ESM modules and TypeScript declarations in dist/`,
  );
}
