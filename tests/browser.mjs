...
await page.route('**/__ribbon_test__', (route) =>
  route.fulfill({
    contentType: 'text/html',
    body: '<!doctype html><html lang="en"><meta charset="utf-8"><title>RibbonWeb browser regressions</title><style>body{margin:20px;font-family:system-ui}#outside{margin:20px}ribbon-web{width:100%}</style><body><button id="outside">Outside ribbon</button></body></html>',
  }),
);
const cases = [];
...
async function setup(initialize) {
  ...
  await page.evaluate(async () => {
    ...
    window.makeRibbon = (options = {}, dataContext) => {
      const homeItems = options.items || [
        {
          id: 'run',
          label: 'Run action',
          icon: 'save',
          keyTip: 'R',
          command: (parameter) => state.calls.push(['run', parameter]),
          commandParameter: 42,
        },
      ];
      const modelOptions = {
        title: 'Regression ribbon',
        theme: 'light',
        tabs: [
          {
            id: 'home',
            header: 'Home',
            keyTip: 'H',
            groups: [{ id: 'main', header: 'Main', items: homeItems }],
          },
          {
            id: 'insert',
            header: 'Insert',
                ...
                items: [
                  {
                    id: 'shape',
                    label: 'Insert shape',
                    command: () => state.calls.push(['shape']),
                  },
                ],
              },
            ],
            ...
            groups: [
              {
                ...
                items: [{ id: 'crop', label: 'Crop', command: () => state.calls.push(['crop']) }],
              },
            ],
          },
        ],
      ...
      for (const name of [
        ...
        'ribbon-change',
        'ribbon-tab-change',
        'ribbon-context-change',
        'ribbon-customization-change',
        'ribbon-backstage-change',
        'ribbon-launcher',
      ])
      ...
      for (const name of ['ribbon-preview', 'ribbon-preview-end'])
        r.addEventListener(name, (e) => state.preview.push({ name, ...e.detail }));
      r.addEventListener('ribbon-error', (e) =>
        state.errors.push({ id: e.detail.id, message: e.detail.error?.message }),
      );
...
test('native custom element renders typed model, tabs, groups and linked ARIA', async () => {
  ...
  assert.equal(
    await attr(page.getByRole('tab', { name: 'Home', exact: true }), 'aria-selected'),
    'true',
  );
  assert.equal(await attr(page.getByRole('tabpanel'), 'aria-labelledby'), 'tab-home');
  assert.equal(
    await attr(page.getByRole('tab', { name: 'Home', exact: true }), 'aria-controls'),
    'panel-home',
  );
...
test('commands execute exactly once with parameters and composed events', async () => {
  await setup(() => {
    makeRibbon();
    document.body.addEventListener('ribbon-command', (e) => (window.bubbledCommand = e.detail.id));
  });
  ...
  assert.equal(
    await page.evaluate(() => state.events.filter((e) => e.name === 'ribbon-command').length),
    1,
  );
});
...
test('RelayCommand CanExecute notifications update disabled state', async () => {
  await setup(() => {
    ...
    window.command = new api.RelayCommand(
      () => state.calls.push('executed'),
      () => allowed,
    );
    makeRibbon({ items: [{ id: 'gated', label: 'Gated action', command }] });
  });
      ...
      items: [
        {
          ...
          command: () => state.calls.push('unexpected'),
        },
      ],
    });
    ribbon.addEventListener('ribbon-command-executing', (e) => e.preventDefault());
  });
...
test('command failures raise ribbon-error without an unhandled browser error', async () => {
  ...
  assert.deepEqual(await page.evaluate(() => state.errors), [
    { id: 'failure', message: 'Expected command failure' },
  ]);
...
test('AsyncRelayCommand prevents duplicate execution while work is pending', async () => {
  await setup(() => {
    window.command = new api.AsyncRelayCommand(
      () =>
        new Promise((resolve) => {
          state.calls.push('started');
          window.finishCommand = resolve;
        }),
    );
    makeRibbon({ items: [{ id: 'async', label: 'Async work', command }] });
  });
...
test('toggle TwoWay MVVM binding synchronizes model and view model', async () => {
  await setup(() => {
    window.vm = new api.ObservableObject({ bold: false });
    makeRibbon(
      {
        items: [
          {
            ...
            type: 'toggle',
            label: 'Bold',
            bindings: { checked: new api.Binding('Bold', { mode: 'TwoWay' }) },
          },
        ],
      },
      vm,
...
test('textbox TwoWay binding updates both directions and command event values', async () => {
  await setup(() => {
    window.vm = new api.ObservableObject({ name: 'Initial' });
    makeRibbon(
      {
        items: [
          {
            ...
            type: 'textbox',
            label: 'Document name',
            bindings: { value: { path: 'Name', mode: 'TwoWay' } },
          },
        ],
      },
      vm,
  ...
  assert.equal(
    await page.evaluate(() => state.events.find((e) => e.name === 'ribbon-command')?.value),
    'New document',
  );
  await page.evaluate(() => {
    vm.Name = 'From view model';
  });
  await flush();
...
test('dropdown selection preserves selected value and change notification', async () => {
  ...
  assert.equal(
    await page.evaluate(() => state.events.find((e) => e.name === 'ribbon-change')?.value),
    'b',
  );
});
      ...
      items: [
        {
          ...
          type: 'combobox',
          label: 'Font family',
          items: [{ label: 'Arial' }, { label: 'Georgia' }],
        },
      ],
    });
  });
      ...
      items: [
        {
          ...
          validationMessage: 'Use a size from 1 to 100',
        },
      ],
    });
  });
...
test('slider previews intermediate values and commits a numeric value', async () => {
  await setup(() => {
    makeRibbon({
      items: [{ id: 'zoom', type: 'slider', label: 'Zoom', value: 20, min: 0, max: 100 }],
    });
  });
  await input('zoom').evaluate((n) => {
    n.value = '65';
...
test('color picker emits live preview and commits chosen color', async () => {
  await setup(() => {
    makeRibbon({ items: [{ id: 'color', type: 'color', label: 'Text color', value: '#000000' }] });
  });
  await input('color').evaluate((n) => {
    n.value = '#336699';
    n.dispatchEvent(new Event('input', { bubbles: true }));
...
test('checkbox state and radio group exclusivity have accessible semantics', async () => {
  await setup(() => {
    makeRibbon({
      items: [
        { id: 'check', type: 'checkbox', label: 'Show grid' },
        { id: 'left', type: 'radio', label: 'Align left', groupName: 'alignment', checked: true },
        { id: 'right', type: 'radio', label: 'Align right', groupName: 'alignment' },
      ],
    });
  });
...
test('menus support keyboard navigation, disabled items, activation and focus return', async () => {
      ...
      items: [
        {
          ...
          label: 'Actions',
          items: [
            { id: 'first', label: 'First', command: () => state.calls.push('first') },
            { id: 'disabled', label: 'Disabled', enabled: false },
            { id: 'last', label: 'Last', command: () => state.calls.push('last') },
          ],
        },
      ],
    });
  ...
  assert.equal(
    await page.evaluate(() => ribbon.shadowRoot.activeElement.dataset.controlId),
    'last',
  );
...
test('nested and dynamic menus use current items on each opening', async () => {
          ...
          getItems: async () => [
            {
              ...
              items: [
                { id: 'leaf', label: 'Open recent', command: () => state.calls.push('recent') },
              ],
            },
          ],
        },
  ...
  await control('dynamic').click();
  assert.equal(
    await page.getByRole('menuitem', { name: 'Generation 2', exact: true }).isVisible(),
    true,
  );
      ...
      items: [
        {
          ...
          type: 'split',
          label: 'Paste',
          command: () => state.calls.push('paste'),
          items: [
            { id: 'paste-text', label: 'Paste text', command: () => state.calls.push('text') },
          ],
        },
      ],
    });
...
test('gallery supports preview/cancel, categories and committed selection', async () => {
      ...
      items: [
        {
          ...
          value: 'plain',
          items: [
            { id: 'plain', label: 'Plain', category: 'Basic' },
            { id: 'title', label: 'Title', category: 'Heading' },
            { id: 'subtitle', label: 'Subtitle', category: 'Heading' },
          ],
          preview: (value) => state.calls.push(['preview', value]),
          cancelPreview: () => state.calls.push(['cancel']),
        },
      ],
    });
  });
  ...
  await page
    .getByRole('combobox', { name: 'Gallery category', exact: true })
      ...
      items: [
        {
          ...
          render: () => {
            ...
            n.textContent = 'Custom action';
            n.onclick = () => state.calls.push('custom');
            return n;
          },
        },
        { id: 'slot', type: 'custom', slot: 'extra' },
      ],
    });
    const n = document.createElement('span');
    n.slot = 'extra';
    n.textContent = 'Slotted content';
    ribbon.append(n);
  });
  await page.getByRole('button', { name: 'Custom action', exact: true }).click();
  assert.deepEqual(await page.evaluate(() => state.calls), ['custom']);
  assert.equal(
    await page.locator('ribbon-web slot').evaluate((n) => n.assignedNodes()[0].textContent),
    'Slotted content',
  );
});
...
test('untrusted titles, labels and gallery previews are rendered as text', async () => {
  await setup(() => {
    ...
    makeRibbon({
      title: payload,
      items: [
        { id: 'xss', label: payload },
        {
          id: 'gallery',
          type: 'gallery',
          label: 'Text previews',
          items: [{ id: 'p', label: payload, preview: payload }],
        },
      ],
    });
  });
...
test('layout, theme and touch density visibly change the control surface', async () => {
  await setup();
  const classicHeight = await page
    .getByRole('tabpanel')
  ...
  assert.ok(
    (await page.getByRole('tabpanel').evaluate((n) => n.getBoundingClientRect().height)) <
      classicHeight,
  );
  assert.equal(
    await page.locator('ribbon-web').evaluate((n) => getComputedStyle(n).colorScheme),
    'dark',
  );
...
test('backstage pages render content, execute actions and return to ribbon', async () => {
      ...
      backstage: [
        {
          ...
          items: [
            { id: 'export', label: 'Export document', command: () => state.calls.push('export') },
          ],
        },
      ],
    });
  ...
  assert.equal(
    await page.getByRole('heading', { name: 'Information', exact: true }).isVisible(),
    true,
  );
...
test('quick access customization adds a command and saves serializable state', async () => {
  await setup(() => {
    makeRibbon({ storageKey: 'ribbon-browser-qat' });
    localStorage.removeItem('ribbon-browser-qat');
  });
  ...
  assert.equal(
    await page
      .getByRole('toolbar', { name: 'Quick access toolbar', exact: true })
      .locator('[data-control-id="run"]')
      .count(),
    1,
  );
  assert.deepEqual(
    await page.evaluate(
      () => JSON.parse(localStorage.getItem('ribbon-browser-qat')).quickAccessToolbar,
    ),
    ['run'],
  );
  await page
...
test('customization dialog applies tab visibility and exported order survives import', async () => {
  ...
  await page.evaluate(() => {
    const data = ribbon.exportCustomization();
    data.hiddenTabs = [];
    data.tabOrder = ['insert', 'home', 'picture'];
    data.tabLabels = { insert: 'Create' };
    data.selectedTab = 'insert';
    ribbon.importCustomization(data);
  });
  await flush();
...
test('group launcher, context menu and selection mini toolbar dispatch commands', async () => {
          ...
          groups: [
            {
              id: 'main',
              header: 'Main',
              launcher: () => state.calls.push('launcher'),
              items: [{ id: 'run', label: 'Run action', command: () => state.calls.push('run') }],
            },
          ],
        },
      ],
  ...
  assert.equal(
    await page.getByRole('toolbar', { name: 'Selection formatting', exact: true }).isVisible(),
    true,
  );
  await page
    .getByRole('toolbar', { name: 'Selection formatting', exact: true })
    .getByRole('menuitem', { name: 'Run action', exact: true })
    .click();
...
test('ObservableCollection additions stay reactive after their first render', async () => {
  await setup(() => {
    window.items = new api.ObservableCollection([{ id: 'first', label: 'First action' }]);
    makeRibbon({ items });
  });
  ...
  await page.evaluate(() => {
    ribbon.GetControl('added').Label = 'Updated after add';
  });
  await flush();
...
test('disconnect releases subscriptions and reattach observes model changes once', async () => {
  await setup(() => {
    window.command = new api.RelayCommand(() => state.calls.push('one'));
    makeRibbon({ items: [{ id: 'run', label: 'Before detach', command }] });
  });
  const before = await page.evaluate(() => command.CanExecuteChanged.count);
  assert.ok(before >= 1);
  await page.evaluate(() => {
    ribbon.remove();
    ribbon.GetControl('run').Label = 'While detached';
  });
...
test('multiple ribbons route a shortcut only to the focused ribbon', async () => {
  await setup(() => {
    window.first = makeRibbon({
      items: [
        {
          id: 'first',
          label: 'First command',
          shortcut: 'Ctrl+k',
          command: () => state.calls.push('first'),
        },
      ],
    });
    window.second = makeRibbon({
      items: [
        {
          id: 'second',
          label: 'Second command',
          shortcut: 'Ctrl+k',
          command: () => state.calls.push('second'),
        },
      ],
    });
    first.after(second);
...
test('shortcuts avoid text inputs unless explicitly allowed', async () => {
  await setup(() => {
    makeRibbon({
      items: [
        { id: 'run', label: 'Run', shortcut: 'Ctrl+k', command: () => state.calls.push('run') },
        { id: 'text', type: 'textbox', label: 'Typing area' },
      ],
    });
  });
...
test('responsive overflow keeps hidden groups and their commands reachable', async () => {
      ...
      tabs: [
        {
          ...
          groups: Array.from({ length: 10 }, (_, i) => ({
            ...
            items: [
              {
                id: `action-${i}`,
                label: `Wide command number ${i}`,
                command: () => state.calls.push(i),
              },
            ],
          })),
        },
  ...
  await flush();
  assert.equal(
    await page.getByRole('button', { name: 'More ribbon groups', exact: true }).isVisible(),
    true,
  );
  assert.ok((await page.locator('ribbon-web .panel > .group[hidden]').count()) > 0);
  const group = await page
    .locator('ribbon-web .panel > .group[hidden]')
    .first()
...
test('markup-only custom element builds groups and controls without an imperative model', async () => {
  await setup(() => {
    document.body.insertAdjacentHTML(
      'afterbegin',
      '<ribbon-web><ribbon-tab id="markup" header="Markup"><ribbon-group id="declarative" header="Declarative"><ribbon-button id="hello" label="Hello markup"></ribbon-button><ribbon-toggle id="toggle" label="Markup toggle"></ribbon-toggle></ribbon-group></ribbon-tab></ribbon-web>',
    );
    window.ribbon = document.querySelector('ribbon-web');
  });
...
test('dynamic tab/group/control APIs normalize typed models and support ordered insertion', async () => {
  ...
  const typed = await page.evaluate(() => {
    ...
    const group = ribbon.AddGroup('tools', {
      Id: 'tools-main',
      Header: 'Tool commands',
      Items: [],
    });
    const added = ribbon.AddControl(
      'tools-main',
      { Id: 'tool-action', Header: 'Dynamic action', Command: () => state.calls.push('dynamic') },
      0,
    );
    return [
      tab instanceof api.RibbonTab,
      group instanceof api.RibbonGroup,
      added instanceof api.RibbonControl,
    ];
  });
  ...
  await page.evaluate(() => {
    ribbon.GetControl('tool-action').Header = 'Dynamic renamed';
  });
  await flush();
  assert.equal(await control('tool-action').textContent(), 'Dynamic renamed');
  await assert.rejects(
    page.evaluate(() => ribbon.addTab({ id: 'tools', header: 'Duplicate' })),
    /Duplicate tab/,
  );
  await assert.rejects(
    page.evaluate(() => ribbon.addGroup('tools', { id: 'tools-main', header: 'Duplicate' })),
    /Duplicate group/,
  );
  await assert.rejects(
    page.evaluate(() => ribbon.addControl('tools-main', { id: 'tool-action', label: 'Duplicate' })),
    /Duplicate control/,
  );
});
...
test('removing selected tabs and controls restores selection and removes QAT references', async () => {
  await setup();
  await page.evaluate(() => {
    ribbon.addToQuickAccess('run');
    ribbon.AddTab({ id: 'temporary', header: 'Temporary', groups: [] });
    ribbon.SelectedTab = 'temporary';
  });
  await flush();
...
test('quick access APIs add, deduplicate, reorder and remove executable commands', async () => {
  await setup();
  await page.evaluate(() => {
    ribbon.setQuickAccess(['run', 'shape', 'run', 'missing']);
    ribbon.moveQuickAccess('shape', 0);
  });
  await flush();
  ...
  assert.deepEqual(
    await qat
      ...
      .evaluateAll((nodes) => nodes.map((n) => n.dataset.controlId)),
    ['shape', 'run'],
  );
  await qat.locator('[data-control-id="shape"]').click();
  assert.deepEqual(await page.evaluate(() => state.calls), [['shape']]);
  await page.evaluate(() => {
    ribbon.removeFromQuickAccess('shape');
    ribbon.addToQuickAccess('run');
  });
  await flush();
  assert.deepEqual(
...
test('.NET semantic property aliases drive visibility, checked state, text and images', async () => {
  await setup(() => {
    makeRibbon({
      items: [
        new api.RibbonToggleButton({
          Id: 'semantic',
          Header: 'Semantic toggle',
          IsChecked: true,
          IsEnabled: false,
          SmallImageSource: 'bold',
          ToolTipTitle: 'Semantic tooltip',
          ToolTipDescription: 'A precise tooltip description',
        }),
        new api.RibbonTextBox({
          Id: 'semantic-text',
          Header: 'Semantic text',
          Text: 'Initial text',
        }),
  ...
  await page.evaluate(() => {
    ribbon.UpdateControl('semantic', {
      IsEnabled: true,
      IsChecked: false,
      Header: 'Updated semantics',
    });
    ribbon.GetControl('semantic-hidden').IsVisible = true;
    ribbon.GetControl('semantic-text').Text = 'Changed text';
  });
  await flush();
...
test('.NET semantic binding keys and PascalCase converters update accessible labels and parameters', async () => {
  await setup(() => {
    window.vm = new api.ObservableObject({
        ...
        items: [
          new api.RibbonToggleButton({
            ...
            Bindings: {
              Header: 'Caption',
              IsEnabled: 'Allowed',
              IsChecked: new api.Binding('Pressed', { Mode: 'TwoWay' }),
              CommandParameter: 'Parameter',
            },
            Command: (parameter) => state.calls.push(parameter),
          }),
          new api.RibbonTextBox({
            Id: 'converted',
            Header: 'Converted title',
            Bindings: {
              Text: new api.Binding('Title', {
                Mode: 'TwoWay',
                Converter: {
                  Convert: (value) => value.toUpperCase(),
                  ConvertBack: (value) => value.toLowerCase(),
                },
              }),
            },
          }),
  ...
  await page.evaluate(() => {
    vm.Caption = 'Updated bound caption';
    vm.Allowed = false;
  });
  await flush();
...
test('replacing ItemsSource preserves selection and subscribes to the new collection', async () => {
  await setup(() => {
    window.oldItems = new api.ObservableCollection([{ label: 'Alpha', value: 'a' }]);
    makeRibbon({
      items: [
        new api.RibbonDropDown({
          Id: 'source',
          Header: 'Options',
  ...
  await page.evaluate(() => {
    window.nextItems = new api.ObservableCollection([{ label: 'Beta', value: 'b' }]);
    ribbon.GetControl('source').ItemsSource = nextItems;
    ribbon.GetControl('source').SelectedValue = 'b';
  });
  await flush();
...
test('nested DataContext replacement detaches the old child and observes the new one', async () => {
  await setup(() => {
    window.oldChild = new api.ObservableObject({ Title: 'Old child' });
    window.vm = new api.ObservableObject({ Editor: oldChild });
    makeRibbon(
      {
        items: [
          {
            ...
            type: 'textbox',
            label: 'Nested title',
            bindings: { value: new api.Binding('Editor.Title', { mode: 'TwoWay' }) },
          },
        ],
      },
      vm,
  ...
  await page.evaluate(() => {
    window.newChild = new api.ObservableObject({ Title: 'New child' });
    vm.Editor = newChild;
  });
  await flush();
  ...
  await page.evaluate(() => {
    newChild.Title = 'New child changed';
  });
  await flush();
...
test('model Context and SelectedTab replacements immediately change contextual UI', async () => {
  await setup();
  await page.evaluate(() => {
    ribbon.Model.Context = { picture: true };
    ribbon.Model.SelectedTab = 'picture';
  });
  await flush();
  assert.equal(await page.getByRole('tab', { name: 'Picture', exact: true }).isVisible(), true);
  assert.equal(await control('crop').isVisible(), true);
  await page.evaluate(() => {
    ribbon.Model.Context = {};
  });
  await flush();
  assert.equal(await page.getByRole('tab', { name: 'Picture', exact: true }).count(), 0);
  assert.equal(await page.evaluate(() => ribbon.Model.SelectedTab), 'home');
...
test('RelayCommand group launchers honor availability and receive the group parameter', async () => {
  await setup(() => {
    window.allowed = false;
    window.launcherCommand = new api.RelayCommand(
      (group) => state.calls.push(group.id),
      () => allowed,
    );
    makeRibbon({
      tabs: [
        {
          id: 'home',
          header: 'Home',
          groups: [{ id: 'launch', header: 'Launch group', launcher: launcherCommand, items: [] }],
        },
      ],
    });
  });
  ...
  await page.evaluate(() => {
    allowed = true;
    launcherCommand.NotifyCanExecuteChanged();
  });
  await flush();
  ...
  assert.equal(
    await page.evaluate(() => state.events.filter((e) => e.name === 'ribbon-launcher').length),
    1,
  );
});
...
test('throwing onChange callbacks are captured as ribbon-error without unhandled rejection', async () => {
      ...
      items: [
        {
          ...
          onChange: () => {
            throw new Error('Expected onChange failure');
          },
          command: () => state.calls.push('must not execute'),
        },
      ],
    });
  });
  ...
  await flush();
  assert.deepEqual(await page.evaluate(() => state.errors), [
    { id: 'throw-change', message: 'Expected onChange failure' },
  ]);
  assert.deepEqual(await page.evaluate(() => state.calls), []);
...
test('nested menu Back keeps its root position and returns focus to the parent trigger', async () => {
      ...
      items: [
        {
          ...
          items: [
            { id: 'other', label: 'Other action' },
            {
              ...
              type: 'menu',
              label: 'Nested branch',
              items: [{ id: 'leaf', label: 'Nested action' }],
            },
          ],
        },
      ],
  ...
  assert.equal(
    await page.getByRole('menuitem', { name: 'Other action', exact: true }).isVisible(),
    true,
  );
  assert.equal(
    await page.evaluate(() => ribbon.shadowRoot.activeElement.dataset.controlId),
...
test('customization names and visibility remain drafts until Apply and Close discards them', async () => {
  ...
  await page.evaluate(() => ribbon.openCustomization());
  assert.equal(
    await page.getByRole('textbox', { name: 'Rename Home', exact: true }).inputValue(),
    'Home',
  );
  assert.equal(
    await page.getByRole('checkbox', { name: 'Show Insert', exact: true }).isChecked(),
    true,
  );
  ...
  await page.getByRole('button', { name: 'Apply', exact: true }).click();
  assert.equal(
    await page.getByRole('tab', { name: 'Applied name', exact: true }).isVisible(),
    true,
  );
  assert.equal(await page.getByRole('tab', { name: 'Insert', exact: true }).count(), 0);
});
...
test('showcase presentation edits slide content and adds, duplicates and deletes slides', async () => {
  ...
  assert.equal(
    await page.locator('[aria-label="Slide title"]').textContent(),
    'Browser-tested title',
  );
  assert.equal(
    await page.locator('[aria-label="Slide subtitle"]').textContent(),
    'Browser-tested subtitle',
  );
...
test('MVVM example synchronizes both views, command availability and async saving', async () => {
  ...
  await control('bold').click();
  assert.equal(
    await page.locator('#preview-body').evaluate((n) => getComputedStyle(n).fontWeight),
    '700',
  );
  ...
  await page.waitForFunction(() =>
    document.querySelector('#save-status').textContent.includes('1 snapshots'),
  );
  assert.equal(await control('save').isDisabled(), true);
  assert.equal(
    await page.evaluate(() => JSON.parse(localStorage.getItem('ribbonweb-mvvm-document')).title),
    'Edited in the ribbon',
  );
});
...
test('RibbonX example boot, callbacks, dynamic menu and tab activation work', async () => {
  ...
  await control('bold').click();
  assert.equal(
    await page.locator('#preview').evaluate((n) => getComputedStyle(n).fontWeight),
    '700',
  );
  await input('font').selectOption('serif');
  assert.match(
    await page.locator('#preview').evaluate((n) => getComputedStyle(n).fontFamily),
    /Georgia/,
  );
...
test('RibbonX browser parser and callback conformance suite', async () => {
  ...
  const results = await page.evaluate(async () =>
    (await import('/tests/ribbonx.browser.js')).runRibbonXTests(),
  );
...
const results = [];
try {
  for (const { name, run } of cases.filter(
    (testCase) =>
      !process.env.RIBBON_TEST_FILTER || testCase.name.includes(process.env.RIBBON_TEST_FILTER),
  )) {
    pageErrors = [];
    ...
    try {
      ...
      if (!page.isClosed())
        await page
          .screenshot({ path: resolve(output, `failure-${results.length}.png`), fullPage: true })
          .catch(() => {});
    }
  }
} finally {
...
console.log(
  `Browser regressions: ${results.length - failures.length}/${results.length} groups passed.`,
);
