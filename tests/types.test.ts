import {
  ...
  RibbonElement,
  normalizeControl,
  createRibbonXAdapter,
  createDotNetBridge,
  EventSource,
  type RibbonControlOptions,
  type RibbonEventMap,
} from '../src/index.js';
...
const command = new RelayCommand(
  (parameter: string) => parameter.length,
...
const toggle = new RibbonToggleButton({
  Label: 'Bold',
  Bindings: { checked: new Binding<boolean, boolean>('bold', { mode: 'TwoWay' }) },
});
...
const gallery = new RibbonGallery({
  Items: [{ label: 'Title', value: 'title', style: { fontSize: '20px' } }],
});
...
source.subscribe((name, value) => name.repeat(value));
source.emit('x', 3);
const adapter = createRibbonXAdapter('<customUI/>', {
  onLoad(api) {
    api.Invalidate();
  },
});
adapter.attach(element).InvalidateControl('save');
const bridge = createDotNetBridge(element, {
  invokeMethodAsync: async (name, payload) => undefined,
});
...
const dotNetButton = new RibbonButton({
  Header: 'Save',
  IsEnabled: true,
  IsVisible: true,
...
const dotNetGroup = new RibbonGroup({
  Header: 'Font',
  IsVisible: false,
  ItemsSource: [dotNetButton],
});
